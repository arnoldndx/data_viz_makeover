...
#rename the rows
rownames(edge_list_quote) <- edge_list_quote$derivative_doc_id
...
for (i in 1:ncol(fp_similarity_mat)){
  ...
  for (j in which(fp_similarity_mat[,i] >0)){
    ...
    if (rownames(fp_similarity_mat)[j] %in% edge_list_quote$derivative_doc_id){
      ...
      if (article_df[colnames(fp_similarity_mat)[i],][2][[1]] == edge_list_quote[rownames(fp_similarity_mat)[j],][2][[1]]){
        ...
        if (edge_list_quote[which(rownames(edge_list_quote) == rownames(fp_similarity_mat)[j]),'primary_doc_id'] != ''){
          edge_list_quote[which(rownames(edge_list_quote) == rownames(fp_similarity_mat)[j]),'primary_doc_id'] <- paste(
            edge_list_quote[which(rownames(edge_list_quote) == rownames(fp_similarity_mat)[j]),'primary_doc_id'],
            ',',
            colnames(fp_similarity_mat)[i])
          } else {
            edge_list_quote[which(rownames(edge_list_quote) == rownames(fp_similarity_mat)[j]),'primary_doc_id'] <- colnames(
              fp_similarity_mat)[i]
          }
          #reset the edge_list_quote rownames to derivative doc_id for referencing, it auto reverts when u load in data
          rownames(edge_list_quote) <- edge_list_quote$derivative_doc_id
...
```{r}
#tidy up the edge list for quoting

for (i in 1:nrow(edge_list_quote)){
  #check if there is more than one primary_doc_id, one article can quote many sources
  if (length(str_split(edge_list_quote[i,'primary_doc_id'],',')[[1]]) > 1){
    for (j in 1:length(str_split(edge_list_quote[i,'primary_doc_id'],',')[[1]])){
      new_row <- c(str_trim(str_split(edge_list_quote[i,'primary_doc_id'],',')[[1]][j]),
                   edge_list_quote$primary_source[i],
                   edge_list_quote$derivative_doc_id[i],
                   edge_list_quote$derivative_source[i],
                   edge_list_quote$type[i])
      #print(new_row)
...
#remove the multi-valued rows
edge_list_quote <- edge_list_quote[-which(str_detect(edge_list_quote$primary_doc_id,',')),]
```

```{r}
#tidy up the edge list and prepare graphics
edge_list <- rbind(edge_list_quote,edge_list_shadow)
